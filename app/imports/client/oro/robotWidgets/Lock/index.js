...
import React, {
  ...
  const Lock = (props) => {
    ...
    const doUnlock = useCallback(async (targetRobotId) => {
      try {
        const result = await unlockRobot({ robotId: targetRobotId });
        openDialog({
    ...
    const handleToggleLock = useCallback(async () => {
      if (!locked) {
        ...
        // Breaking someone else's lock. Confirm first, and refuse if the selected robot changed
        // while the confirmation was open.
        const targetRobotId = robotId;
        openDialog({
